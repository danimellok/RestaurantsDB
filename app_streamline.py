...
### VIEW REVIEWS FUNCTION
        ...
        # Menu Items Section
        add_menu = st.checkbox("Add menu items", key="add_menu_items_checkbox")
        if add_menu:
            st.subheader("Menu Items")
            dish_name = st.text_input("Enter dish name:")
            dish_rating = st.slider("Enter dish rating (1-5):", 1, 5, key="dish_rating_slider")
            notes = st.text_area("Enter any notes about the dish:")
            if st.button("Add Menu Item", key="add_menu_item_button"):
                # Append to session state menu_items
                st.session_state.menu_items.append({"DishName": dish_name, "DishRating": dish_rating, "Notes": notes})
            ...
            if st.session_state.menu_items:
                ...
                for item in st.session_state.menu_items:
                    st.write(f"- {item['DishName']} (Rating: {item['DishRating']}) - Notes: {item['Notes']}")
        ...
        if st.button("Submit Visit", key="submit_visit_button"):
            result = add_restaurant_visit(
                ...
                visit_date, visit_price, reservation, overall_rating, food_rating, service_rating, st.session_state.menu_items
            )
            st.success(result)
